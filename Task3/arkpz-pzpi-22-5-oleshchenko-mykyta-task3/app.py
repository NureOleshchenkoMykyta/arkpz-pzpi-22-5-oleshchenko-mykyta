...
def login():
    ...
    if account:
        account_id, role = account[0]
        session['user_id'] = account_id
        session['role'] = role
        return jsonify({"message": "Вхід успішний", "role": role}), 200
    else:
        return jsonify({"message": "Неправильный email чи пароль"}), 400
          ...
          items:
            ...
            properties:
              ...
              Text:
                type: string
    """
    # Получение e-mail из строки запроса
    email = request.args.get('email')

    if not email:
        return jsonify({"message": "E-mail не указан"}), 400

    try:
        # Поиск AccountID по e-mail
        account_query = "SELECT AccountID FROM analysisstate.account WHERE Email = %s;"
        account_result = execute_query(account_query, (email,))

        if not account_result:
            return jsonify({"message": "Акаунт с указанным e-mail не найден"}), 404

        account_id = account_result[0][0] if isinstance(account_result[0], tuple) else account_result[0]["AccountID"]

        # Получение нотаток из таблицы notes
        notes_query = "SELECT * FROM analysisstate.notes WHERE AccountID = %s;"
        notes = execute_query(notes_query, (account_id,))

        return jsonify(notes), 200

    except Exception as e:
        return jsonify({"message": f"Помилка отримання нотаток: {str(e)}"}), 500
